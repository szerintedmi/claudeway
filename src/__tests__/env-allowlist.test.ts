...
describe('processIdentityKey', () => {
  it('produces different keys for different users', () => {
    ...
    const key1 = processIdentityKey('U001', perms, config, 'C001');
    const key2 = processIdentityKey('U002', perms, config, 'C001');
  ...
  it('produces different keys for different permissions', () => {
    const config = makeConfig();
    const key1 = processIdentityKey('U001', new Set(['git', 'jiraWrite']), config, 'C001');
    const key2 = processIdentityKey('U001', new Set(), config, 'C001');
    expect(key1).not.toBe(key2);
  });
  ...
  it('produces different keys when env exposure changes', () => {
    ...
    const perms = new Set(['git']);
    const key1 = processIdentityKey('U001', perms, config1, 'C001');
    const key2 = processIdentityKey('U001', perms, config2, 'C001');
    expect(key1).not.toBe(key2);
  });
  ...
  it('includes sorted env var names in the key', () => {
    ...
    const key = processIdentityKey('U001', new Set(['jiraWrite']), config, 'C001');
