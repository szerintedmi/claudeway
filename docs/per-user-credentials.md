...
- Submodule paths are linked automatically as read-only shared references: `git worktree add` doesn't recurse into submodules, so after creating (or reusing) a worktree the gateway replaces unpopulated submodule dirs with symlinks to the main checkout's synced submodules. This avoids per-thread network clones and preserves shared history for read/search/blame/log use. The agent is prompted not to edit those paths; `syncRepos()` updates the shared source checkout.
